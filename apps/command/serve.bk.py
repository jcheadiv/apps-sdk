...
class GriffinRequests(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def handle_one_request(self):
        ...
        logging.error(self.requestline)
        actual_path = urllib.splitquery(self.path)[0]
        if actual_path in self.routes:
            return getattr(self, 'handle_' + self.routes[actual_path])()
        method()
    ...
    # def handle_pass(self):
    #     global rnum
    #     params = dict([(x.split('=')[0], x.split('=')[1]) for x in
    #                    urllib.splitquery(self.path)[1].split('&')])
    ...
    #         rnum += 1
    #         local_rnum = rnum
    #         complete[local_rnum] = req

    #     while 1:
    #         if local_rnum != -1 and local_rnum in complete:
    #             self._respone = complete.pop(local_rnum)
    #             break
    #         if len(outstanding[params['type']]) > 0:
    #             self._response = outstanding[params['type']].pop(0)
    #             break
    ...
    def handle_browser(self):
        req = self.rfile.read(int(self.headers['Content-Length']))
        outstanding['client'].append(req)
        while 1:
            if len(outstanding['browser']) > 0:
                break
            time.sleep(random.random())

        self._response = outstanding['browser'].pop(0)
        self.send_message()
