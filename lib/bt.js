...
_.extend(bt, {
  ...
  _handlers: {
    ...
    torrent: function(resp) {
       ...
       *     2 - completed
       */
      $("body").trigger('torrentStatus', resp);
  ...
  template: function(tmpl_name, vals) {
    ...
    if (!(tmpl_name in bt._templates))
      var resource = bt.resource(sprintf('html/%s.html', tmpl_name));
      if ('undefined' === typeof resource) {
        throw Error(sprintf('Template %s does not exist.', tmpl_name));
      }
      bt._templates[tmpl_name] = _.template(resource);
...
(function() {
  ...
  $(document).ready(function() {
    ...
    $(bt._page_element).html(bt.resource(bt._index_content));
  });
})();
