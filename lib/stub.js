...
var stub = {
  _init: function() {
    ...
    if (!stub.stash.get('__computer_id'))
      stub.stash.set('__computer_id',
                     Math.floor(Math.random()*Math.pow(10,30)).toString(36));
    stub.settings.set(
      'computer_id', stub.stash.get('__computer_id'));
  ...
  create: function(name, path, callback) {
    var tor = new bt._objs.Torrent('', {
      info: {
        files: _.map(path, function(v) {
          return {
            path: [v],
            length: 0
          }
  ...
  browseforfolder: function(callback) {
    if (!callback) return;
    callback('C:\\'+GetRandomPathText(1, 100) + '\\' + GetRandomPathText(1, 100) + '\\');
  },
  browseforfiles: function(callback) {
    if (!callback) return;
    callback(['C:\\'+GetRandomPathText(1, 100) + '\\' + GetRandomPathText(1, 100) + '.' + GetRandomPathText(3, 3)]);
  }
};

function GetRandomPathText(minsize, maxsize)
{
  var count, actualsize;
  var startvalid = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  var validchars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 $%'`-@{}~!#()&_^+,.=[]";
  actualsize = Math.floor(Math.random() * (maxsize - minsize + 1)) + minsize;
  var text = startvalid.charAt(Math.floor(Math.random() * startvalid.length));
